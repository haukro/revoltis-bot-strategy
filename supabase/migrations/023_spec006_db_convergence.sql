-- Synced from live Supabase migration 20260925093242 (spec006_db_convergence).
-- IMPLEMENTATION-SPEC-006. Runtime B remains disabled.

-- SPEC-006 DB/repo convergence after superseded hardening was applied.
-- Keep one canonical hidden-cycle guard and preserve active recovery ENTRY fencing.

drop trigger if exists paper_spec006_guard_shadow_cycle_identity_trg
on public.paper_strategy_shadow_state;
drop function if exists public.paper_spec006_guard_shadow_cycle_identity();
...
begin
  ...
  if found then
    return jsonb_build_object(
      'resumed',false,
      'reason',case when v_existing.status='CRITICAL'
        then 'CRITICAL_OWNER_BLOCKS_RECOVERY'
        else 'WAIT_EXISTING_OWNER' end,
