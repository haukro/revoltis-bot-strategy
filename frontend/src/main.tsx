...
function App() {
  ...
  const proposeUniverse = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/market/universe/pick', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ timeframe, max_picks: 5, shortlist_size: 30, deep_scan_size: 20, min_atr_ratio: 0.0015, max_atr_ratio: 0.03, trade_notional_usdt: Number(config.stake_amount || 50), lock_hours: 24 }) });
