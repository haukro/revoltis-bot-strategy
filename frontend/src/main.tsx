...
function App() {
  ...
  const scanOkx = async () => {
    ...
    try {
      const response = await fetch('/api/market/scan?quote=USDT&limit=30');
  ...
  const proposeUniverse = async () => {
    ...
    setUniverseLoading(true); setUniverseProposal(null); setMessage('Auto Market Scanner prechádza celý OKX USDT trh…');
    try {
      const timeframe = String(config.timeframe) === '15m' ? '15m' : '5m';
      const response = await fetch('/api/market/universe/pick', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ timeframe, max_picks: 5, shortlist_size: 30, deep_scan_size: 12, trade_notional_usdt: Number(config.stake_amount || 50), lock_hours: 24 }) });
      if (!response.ok) throw new Error(await response.text());
      const proposal = await response.json(); setUniverseProposal(proposal);
      setMessage(proposal.status === 'ok' ? 'Top 5 trhov je pripravených na 24 h lock.' : proposal.message);
  ...
  const availableCoins = useMemo(() => universeLocked
    ? [...(activeUniverse?.pairs || selected)]
    : Array.from(new Set([...coins, ...selected, ...(universeProposal?.picks || []).map((item: any) => item.pair)])),
    [selected, activeUniverse, universeProposal, universeLocked]);
...
function BotControlCenter({ scanner, error, loading, running, onRefresh, onToggle, onChoose, dailyLoss, stake, preferredPairs }: { scanner: any, error: string, loading: boolean, running: boolean, onRefresh: () => void, onToggle: () => void, onChoose: (pair: string) => void, dailyLoss: number, stake: number, preferredPairs: string[] }) {
  ...
  return <section className={`bot-control ${running ? 'bot-running' : ''}`}>
    <div className="bot-control-head"><div><span className="eyebrow">TRHY</span><h3>Vyber a otestuj</h3><p>OKX spot páry. Reálne obchodovanie je vypnuté.</p></div><div className="bot-head-actions"><button className="ghost" onClick={onRefresh} disabled={loading}>{loading ? 'Obnovujem…' : 'Obnoviť'}</button><button className={running ? 'stop-button' : 'run-button'} onClick={onToggle} disabled={!best}>{running ? '■ Stop' : '▶ Sledovať'}</button></div></div>
