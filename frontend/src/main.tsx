...
function App() {
  ...
  const runSimulation = async () => {
    ...
    if (historicalMode) {
      ...
      try {
        const response = await fetch('/api/simulations/run', { method: 'POST', signal: controller.signal, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ settings: currentConfig, candle_limit: Math.min(45000, Math.max(100, wantedCandles)), start_time: startTime, end_time: endTime }) });
  ...
  return <main>
    ...
    <section className="optimizer-panel">
      <div className="optimizer-head"><div><span className="eyebrow">AI OPTIMALIZAČNÝ AGENT</span><h3>Hľadanie najlepšej stratégie</h3><p>Agent otestuje všetkých 14 coinov, všetky 4 intervaly sviečok a viac variantov nastavení. Výsledok overí na oddelenej časti dát.</p></div><button onClick={startOptimizer} disabled={optimizerRunning}>{optimizerRunning ? 'Agent pracuje…' : '✦ Spustiť AI optimalizáciu'}</button></div>
...
function FreqtradeVisual() { return <section className="freqtrade-visual"><div className="ft-brand"><div className="ft-logo">F</div><div><span>CIEĽOVÁ OBCHODNÁ PLATFORMA</span><h3>Freqtrade</h3><p>Bezplatný Python bot · vlastný server · úplná kontrola stratégie</p></div><i>PRIPRAVENÉ</i></div><div className="ft-flow"><article><b>R</b><span>Revoltis</span><small>simulácia a AI optimalizácia</small></article><em>→</em><article><b>PY</b><span>Algoritmus</span><small>RevoltisAIOptimized_v1.py</small></article><em>→</em><article className="highlight"><b>F</b><span>Freqtrade</span><small>backtest a bezpečný dry-run</small></article><em>→</em><article><b>₿</b><span>Binance Spot</span><small>trhové dáta a neskôr príkazy</small></article></div><div className="ft-bottom"><div><span>EXPORTNÝ BALÍK</span><ul><li><b>01</b> Stratégia Python</li><li><b>02</b> Dry-run konfigurácia</li><li><b>03</b> Zoznam coinov</li><li><b>04</b> Optimalizačný report</li></ul></div><div className="ft-safety"><span>BEZPEČNOSTNÁ CESTA</span><p><b>Backtest</b><i>→</i><b>Dry-run</b><i>→</i><b>Kontrola výsledkov</b><i>→</i><b>Voliteľný live režim</b></p><small>Žiadne API kľúče ani reálne obchodovanie nie sú súčasťou exportu zo simulácie.</small></div></div></section>; }
function AlgorithmResult({ result, onCopy }: { result: any, onCopy: () => void }) { const m = result.validation_metrics || {}; const parameterKeys = ['bb_period', 'bb_deviation', 'rsi_period', 'rsi_oversold', 'atr_period', 'atr_min_percent', 'atr_max_percent', 'min_volume_ratio', 'rebound_min_percent', 'rebound_max_percent', 'stop_loss_percent', 'trailing_start_percent', 'trailing_distance_percent']; return <div className="algorithm-result"><div className="result-title"><div><span>NAJLEPŠÍ OVERENÝ VARIANT</span><h4>{result.pair} · {result.timeframe}</h4><p>{result.method} · {result.tested_combinations} otestovaných kombinácií</p></div><button onClick={onCopy}>Kopírovať algoritmus</button></div><div className="result-metrics"><div><span>Zisk/strata overenia</span><b className={Number(m.realized_profit) >= 0 ? 'positive' : 'negative'}>{Number(m.realized_profit || 0) >= 0 ? '+' : ''}{m.realized_profit || 0} USDT</b></div><div><span>Max. drawdown</span><b>{m.max_drawdown_percent || 0} %</b></div><div><span>Obchody</span><b>{m.closed_trades || 0}</b></div><div><span>Úspešnosť</span><b>{m.win_rate || 0} %</b></div><div><span>Skóre</span><b>{result.score}</b></div></div><details open><summary>Výsledné nastavenia <span>⌄</span></summary><div className="parameter-grid">{parameterKeys.map(key => <div key={key}><span>{fields[key]}</span><b>{String(result.settings?.[key])}</b></div>)}</div></details><details><summary>Kopírovateľný Freqtrade algoritmus <span>+</span></summary><pre>{result.strategy_code}</pre></details><small className="cost-model">Model nákladov: {result.cost_model}</small></div>; }
...
function TradeList({ trades }: { trades: any[] }) { return trades.length ? <ul className="list">{trades.slice(0, 5).map(trade => <li key={trade.id}><span><b>{trade.pair}</b><small>{trade.status}</small></span><b className={Number(trade.profit_usdt) >= 0 ? 'positive' : 'negative'}>{Number(trade.profit_usdt) >= 0 ? '+' : ''}{trade.profit_usdt ?? 0}</b></li>)}</ul> : <div className="empty"><span>◎</span><p>Zatiaľ bez simulovaných obchodov.</p></div>; }
