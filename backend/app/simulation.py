...
def simulate(candles_by_pair: dict[str, list[dict[str, Any]]], settings: dict[str, Any], fee: float = 0.001) -> dict[str, Any]:
    ...
    for close_time, pair, index in events:
        ...
        for position in current_positions:
            ...
            if exit_reason:
                ...
                trades.append({"id": f"simulation-{pair}-{close_time}-{len(trades)}", "pair": pair, "status": "closed", "opened_at": position["opened_at"], "closed_at": _iso(close_time), "entry_rate": position["entry_rate"], "exit_rate": close, "stake_amount": position["stake"], "profit_usdt": round(profit, 6), "exit_reason": exit_reason, "raw": {"source": "binance_public_candles"}})
        ...
        cash -= stake
        daily_entries[day] += 1
        positions.append({"pair": pair, "entry_rate": close, "stake": stake, "opened_at": _iso(close_time), "high_watermark": close})
        equity_curve.append({"time": _iso(close_time), "value": round(cash + sum(item["stake"] for item in positions), 4)})
    ...
    for point in equity_curve:
        ...
        drawdown = max(drawdown, (peak - point["value"]) / peak * 100 if peak else 0)
    ...
    for pair in candles_by_pair:
        ...
        pair_peak, pair_drawdown = initial_capital, 0.0
