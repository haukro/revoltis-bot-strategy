...
Status: **LOCK CANDIDATE AFTER GROK REVIEW — NOT RUN**
...
Signals while a position is already open are ignored for entry and do not reverse or modify the open position. When flat, the next valid signal is eligible. If both long and short conditions were ever true on the same 1h candle, that signal is discarded.
...
After the specification is locked, but before the official run, data availability and continuity must be certified without computing Strategy C PnL, expectancy, PF, side PnL, drawdown, or PASS/FAIL.

Pre-fold BTC bars may seed range and ATR only.

Warm-up is exactly **24 fully closed 1h BTC bars before the first eligible signal hour**. These bars may initialize the 24-bar breakout range and Wilder ATR only; they cannot generate trades.

No trade may have entry_time before fold_start.

Signals whose exact required entry is after fold_end are not taken.

Before the official run, the continuity certificate must record:

- actual vs expected 5m BTC bar count for warmup + fold
- actual vs expected 1h BTC bar count after UTC aggregation
- any 5m or 1h timestamp gaps
- whether every eligible signal has its exact required 5m entry bar
- final 5m close validity
- no interpolation, forward-fill or delayed replacement fill

A gap that removes a required 5m entry bar invalidates the official run.
...
TEST-SPEC-003 must be implemented in a **separate Strategy C engine / runner path**. It may copy the locked mechanics of TEST-SPEC-002, but implementation work for C must not alter the frozen B engine defaults, B forward-test semantics, or B official runner. Shared helpers are allowed only if their use cannot change TEST-SPEC-002 behavior.
...
## 13. Grok review disposition before lock

External review was received before any TEST-SPEC-003 performance run.

Accepted:

- C1 end_of_test bookkeeping
- C2 exact warmup/fold boundaries
- C3 continuity certificate after spec lock but before official run
- C4 zero-loss PF semantics
- C5 signal handling with max one open position
- C6 Strategy C implementation must not mutate frozen Strategy B behavior

No alpha parameter, market, official fold, cost snapshot, PASS/FAIL threshold, or filter was changed by the review.
